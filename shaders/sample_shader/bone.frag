...
{
    Light temp;
    temp.ambient = light.ambient * texture(material.texture_diffuse1, TexCoords).rgb;

    float diff = max(dot(lightDir, normal), 0.0f);
    temp.diffuse = light.diffuse * diff * texture(material.texture_diffuse1, TexCoords).rgb;

    vec3 viewDir = normalize(viewPos - fragPos);
    vec3 reflectDir = reflect(-lightDir, normal);
    vec3 halfwayDir = normalize(lightDir + viewDir);
    float spec = pow(max(dot(normal, halfwayDir), 0.0f), 32.0f);
    temp.specular = light.specular * spec * texture(material.texture_specular1, TexCoords).rgb;
