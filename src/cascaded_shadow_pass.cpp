    ...
    : m_matricesUBO(0), m_csmDataUBO(0), m_shadowMapFBO(0), m_shadowMap(0), m_lightDir(glm::vec3(0.0f, -1.0f, 0.0f)), m_width(width), m_height(height)
{
    ...
    m_offsetTexture = new ShadowMapOffsetTexture(8, 5, 1.0f);
}
...
glm::mat4 CascadedShadowPass::getLightSpaceMatrix(const float nearPlane, const float farPlane)
{
    ...
    const auto lightView = glm::lookAt(center - m_lightDir * 1.0f, center, glm::vec3(0.0f, 1.0f, 0.0f));
    ...
    // const float zPaddingFactor = 1.0f; // padding ratio
    // float zRange = maxZ - minZ;
    // float padding = zRange * zPaddingFactor;
    ...
    glm::vec3 lsMin = glm::vec3(std::numeric_limits<float>::max());
    glm::vec3 lsMax = glm::vec3(std::numeric_limits<float>::lowest());
    for (const auto& c : corners)
    {
        glm::vec3 trf = glm::vec3(lightView * c);
        lsMin = glm::min(lsMin, trf);
        lsMax = glm::max(lsMax, trf);
    }
    glm::vec3 extents = lsMax - lsMin;
    glm::vec3 texelSize = extents / float(SHADOW_MAP_RESOLUTION);
    lsMin.x = floor(lsMin.x / texelSize.x) * texelSize.x;
    lsMin.y = floor(lsMin.y / texelSize.y) * texelSize.y;
    lsMax.x = lsMin.x + extents.x;
    lsMax.y = lsMin.y + extents.y;

    // 7. Z padding (depth precision 확보)
    const float zPadding = 100.0f; // 필요시 조정
    lsMin.z -= zPadding;
    lsMax.z += zPadding;

    // 8. Light orthographic projection
    glm::mat4 lightProjection = glm::ortho(lsMin.x, lsMax.x, lsMin.y, lsMax.y, lsMin.z, lsMax.z);
...
std::vector<glm::mat4> CascadedShadowPass::getLightSpaceMatrices()
{
    ...
    const float maxCascadeFar = 1000.0f;
    ...
    for (size_t i = 0; i < m_shadowCascadeLevels.size() + 1; ++i)
    {
        ...
        if (i == 0) {
            ...
            farPlane = std::min(m_shadowCascadeLevels[i], maxCascadeFar);
        }
        else if (i < m_shadowCascadeLevels.size()) {
            nearPlane = m_shadowCascadeLevels[i - 1];
            farPlane = std::min(m_shadowCascadeLevels[i], maxCascadeFar);
        }
        else {
            nearPlane = m_shadowCascadeLevels[i - 1];
            farPlane = std::min(camera->farPlane, maxCascadeFar);
        }

        ret.push_back(getLightSpaceMatrix(nearPlane, farPlane));
    }
    return ret;
}
...
std::vector<float> computeCascadeSplits(float nearPlane, float farPlane, size_t cascadeCount)
{
    ...
    for (size_t i = 1; i <= cascadeCount; ++i)
    {
        ...
        float splitDist = lambda * logSplit + (1.0f - lambda) * uniformSplit;
