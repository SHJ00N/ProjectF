...
{
    DefaultCamera.Update(player->ObjectTransform.position, dt);
    glm::mat4 projection = glm::perspective(glm::radians(DefaultCamera.fov), (float)Width / (float)Height, 0.1f, 5000.0f);
...
void Game::ProcessInput(float dt)
{
    ...
    if(Keys[GLFW_KEY_W])
    {
        player->Move(FORWARD, DefaultCamera, curPlayerChunk, dt);
        player->Animator3D.PlayAnimation(&ResourceManager::GetAnimation("knight_run"));
    } 
    if(Keys[GLFW_KEY_S])
    {
        player->Move(BACKWARD, DefaultCamera, curPlayerChunk, dt);
        player->Animator3D.PlayAnimation(&ResourceManager::GetAnimation("knight_run"));
    }
    if (Keys[GLFW_KEY_A])
    {
        player->Move(LEFT, DefaultCamera, curPlayerChunk, dt);
        player->Animator3D.PlayAnimation(&ResourceManager::GetAnimation("knight_run"));
    }   
    if (Keys[GLFW_KEY_D])
    {
        player->Move(RIGHT, DefaultCamera, curPlayerChunk, dt);
        player->Animator3D.PlayAnimation(&ResourceManager::GetAnimation("knight_run"));
    }
